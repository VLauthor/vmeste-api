...
const auth_service_1 = require("../database/auth.service");
    ...
    constructor(d, v, h, m, tokens) {
        ...
        this.loginUser = async (res, dto) => {
            ...
            if (resultMail) {
                ...
                throw new common_1.BadRequestException('Неверный пароль. Пожалуйста проверьте его и повторите попытку.');
            }
            throw new common_1.BadRequestException('Неверная почта. Пожалуйста проверьте ее и повторите попытку.');
        };
        this.signInUser = async (res, dto) => {
            ...
            if (!this.v.password(dto.password))
                throw new common_1.BadRequestException('Ваш пароль слишкм легкий, пожалуйста усложните его.');
            if (!this.v.date(dto.date))
                throw new common_1.BadRequestException('Введена не кореектная дата рождения, пожалуйста проверьте её.');
        ...
        this.test = async (res) => {
            await this.m.sendVerificationMail('shandybin.ion@icloud.com', 'https://ya.ru/');
...
exports.AuthService = AuthService = __decorate([
    ...
    __metadata("design:paramtypes", [auth_service_1.AuthDatabaseService,
